...
class Theme:
    ...
    def stylesheet(*, dark: bool, accent: str, radius: int = 8, scale_factor: float = 1.0) -> str:
        ...
        * {{ font-family: 'Inter','Segoe UI','Roboto','Arial'; font-size:{font_size}pt; color:{text}; }}
        ...
        QTabBar::tab {{ background:{alt}; border:1px solid {border}; padding:{tab_padding_v}px {tab_padding_h}px; margin:{tab_margin}px;
                        border-top-left-radius:{radius}px; border-top-right-radius:{radius}px; }}
        QTabBar::tab:selected {{ background:{panel}; border-bottom-color:{panel}; border-top:2px solid {accent}; color:{text}; }}
        QTabBar::tab:hover:!selected {{ background:{raised}; }}
...
class MangaTextTool(QMainWindow):
    ...
    def _build_right_panel(self):
        ...
        try:
            ...
            self.prop_dock.topLevelChanged.connect(self._on_prop_top_level_changed)
    ...
    def _import_from_typertools(self, data: dict):
        ...
        for style in data.get("styles", []):
            ...
            ts = ts_ranges[0].get("textStyle", {}) if ts_ranges else {}
            ps = ps_ranges[0].get("paragraphStyle", {}) if ps_ranges else {}

            font_family = (ts.get("fontPostScriptName") or ts.get("fontName") or "Arial").strip()
            font_size   = max(6, int(round(ts.get("size", 16))))
            bold        = bool(ts.get("syntheticBold",  False))
            italic      = bool(ts.get("syntheticItalic", False) or ts.get("italics", False))
            fill_color  = _hex_from_ps(ts["color"]) if "color" in ts else "#000000"
